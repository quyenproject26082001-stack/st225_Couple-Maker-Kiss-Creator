...
class CustomizeCharacterViewModel : ViewModel() {
    ...
    suspend fun setBottomNavigationListDefault(targetLayerIndex: Int = -1) {
        ...
        if (itemNavList.size > firstLayerIndex) {
            ...
            android.util.Log.d("GenderSwitch", "setBottomNavigationListDefault: firstLayerIndex=$firstLayerIndex, alreadySelected=$alreadySelected")
            if (!alreadySelected) {
    ...
    suspend fun setFocusItemNavDefault() { itemNavList.forEachIndexed { parentIndex, itemParent ->
        ...
        itemNavList[0] = itemNavList.first().mapIndexed { index, item ->
